...
export class StatementParser {
  ...
  parse(text: string): ParsedStatement {
    ...
    if (verbListMatch) {
      ...
      if (this.lexicon.verbs.has(verb1.toLowerCase()) && this.lexicon.verbs.has(verb2.toLowerCase())) {
        // Recursively expand the object if it contains conjunctions
        const objectExpansions = /\b(and|or)\b/i.test(object)
        ...
        const verb2Expansions = objectExpansions.map(exp => exp.replace(new RegExp(`^${verb1}\\s+`, 'i'), `${verb2} `))
    ...
    if (/\b(and|or)\b/i.test(text) && !this.isVerbConjunction(text)) {
  ...
  private expandRawText(text: string): string[] {
    ...
    if (verbObjectComplement) {
      ...
      if (this.lexicon.verbs.has(verb.toLowerCase()) && /\b(and|or)\b/i.test(comp)) {
        // Expand the complement
        const compVariations = this.expandPhrase(comp)

        // Expand the object if it has conjunctions
        const objVariations = /\b(and|or)\b/i.test(obj) ? this.expandPhrase(obj) : [obj]
  ...
  /**
   * Expand a phrase containing "or" or "and" conjunctions
  ...
  private expandPhrase(phrase: string): string[] {
    if (!phrase || !/\b(and|or)\b/i.test(phrase)) {
      return [phrase]
    }
