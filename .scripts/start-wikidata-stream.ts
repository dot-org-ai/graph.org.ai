...
/**
 * Start Wikidata Streaming (Fire-and-Forget)
 *
...
async function main() {
  console.log('\n🌐 Starting Wikidata Stream to ClickHouse Cloud\n');
  ...
  try {
    ...
    const hasStaging = tableList.some((t: any) => t.name === 'wikidata_staging');
    const hasThings = tableList.some((t: any) => t.name === 'things');

    if (!hasStaging || !hasThings) {
      console.log('❌ Tables not found. Run setup first:');
    ...
    await client.exec({
      query: `
        ...
        SELECT
          trimBoth(trim(TRAILING ',' FROM json)) as entity
        FROM url(
    ...
    console.log('   - Check row count: SELECT count() FROM public.wikidata_staging');
