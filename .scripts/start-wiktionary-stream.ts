...
/**
 * Start Wiktionary Streaming (Pre-processed JSONL from kaikki.org)
 *
 * Submits the long-running INSERT query to ClickHouse and disconnects
 * The query continues running server-side
 */
...
async function main() {
  console.log('\n📚 Starting Wiktionary Stream to ClickHouse Cloud\n');
  ...
  try {
    ...
    const hasStaging = tableList.some((t: any) => t.name === 'wiktionary_staging');

    if (!hasStaging) {
      console.log('❌ wiktionary_staging table not found. Creating it...\n');
      await client.exec({
        query: `
          CREATE TABLE IF NOT EXISTS public.wiktionary_staging (
            line String CODEC(ZSTD)
          ) ENGINE = MergeTree()
          ORDER BY tuple()
          SETTINGS index_granularity = 8192
        `
      });
      console.log('✅ Created wiktionary_staging table\n');
    }
    ...
    await client.exec({
      query: `
        INSERT INTO public.wiktionary_staging
        SELECT *
        FROM url(
    ...
    console.log('   npx tsx .scripts/verify-wiktionary-status.ts\n');
